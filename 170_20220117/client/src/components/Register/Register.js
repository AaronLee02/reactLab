...
class Register extends Component {
    ...
    submitClick = async (type, e) => {
        ...
        if (this.fnValidate()) {
            ...
            axios.post('fund/1', {
                useremail: this.email_val_checker + '@' + this.email2_val_checker
            }, console.log('hellow3'))
                .then(response => {
                    try {
                        console.log('hellow4')
                        console.log('hellow5', response.data)
        ...
        this.fnSignInsert = async (type, e) => {
            ...
            console.log('제이슨 str:',jsonstr)
            console.log('제이슨 str:',jsonstr[1])
            console.log('비번:',this.pwd_val_checker,this.email_val_checker)
            for(let i=0;i<Json_form.length;i++){
                console.log('제이슨 데이터:',i, Json_form[i])
            }
            axios.post('fund/register', {
                useremail: (this.email_val_checker + '@' + this.email2_val_checker),
                username:  this.name_val_checker, userorg:  this.org_val_checker, userpassword: this.pwd_val_checker, usermajor:  this.major_val_checker,
                userphone:this.phone1_val_checker +this.phone2_val_checker +this.phone3_val_checker
            }).then(response => {
